...
const ListItemTrend = ({ data, className, ...props }: IProps) => (
	<ListItemButton
		component={Link}
		href={'/search?q=trend'}
		className={cn(className, 'flex flex-col items-start justify-center')}
		{...props}
	>
		<Typography className={cn('text-sm')} children={data.title} />
		<Typography className={cn('text-sm')} children={data.user.name} />
		<Typography className={cn('text-sm')} children={`${data.posts} posts`} />
	</ListItemButton>
);
